 ...
 * Provides thread-safe access to properties and XML string resources, with error logging via LogClass.
 */
public class ConfigReader {
        ...
        synchronized (ConfigReader.class) {
            ...
            try (InputStream input = ConfigReader.class.getClassLoader().getResourceAsStream("config.properties")) {
                if (input != null) {
                    ...
                    LogClass.info("Loaded config.properties from classpath");
                } else {
                    // Fallback to file system
                    try (InputStream fis = new java.io.FileInputStream(CONFIG_PATH)) {
                        properties.load(fis);
                        LogClass.info("Loaded config.properties from file system: " + CONFIG_PATH);
                    } catch (IOException e) {
                    	LogClass.error("Failed to load config.properties from file system: " + CONFIG_PATH);
                    }
                }
            } catch (IOException e) {
            	LogClass.error("Failed to load config.properties from classpath");
            }
            ...
            try (InputStream input = ConfigReader.class.getClassLoader().getResourceAsStream(STRINGS_PATH)) {
                if (input != null) {
                    ...
                    LogClass.info("Loaded strings.xml from classpath: " + STRINGS_PATH);
                } else {
                	LogClass.warn("strings.xml not found in classpath: " + STRINGS_PATH);
                }
            } catch (IOException e) {
            	LogClass.error("Failed to load strings.xml from classpath: " + STRINGS_PATH);
            }
        }
    }
    ...
    public static String getProperty(String key) {
        if (key == null || key.trim().isEmpty()) {
            LogClass.warn("Property key is null or empty");
            return null;
        }
        String value = properties.getProperty(key);
        if (value == null) {
            LogClass.warn("Property not found for key: " + key);
        } else if (value.trim().isEmpty()) {
            LogClass.warn("Property value is empty for key: " + key);
        }
        return value;
    }
    ...
    public static String getStringResource(String key) {
        if (key == null || key.trim().isEmpty()) {
            LogClass.warn("String resource key is null or empty");
            return null;
        }
        String value = stringResources.get(key);
        if (value == null) {
            LogClass.warn("String resource not found for key: " + key);
        }
        return value;
    }
    ...
    private static HashMap<String, String> parseStringXML(InputStream file) {
        ...
        try {
            ...
            LogClass.error("Failed to parse strings.xml");
        }
        return stringMap;
    }
}